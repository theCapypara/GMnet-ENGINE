...
**
**      Syncs udphp's player list with the player map of htme.
**      We need to make sure, the playermap contains all players from the 
**      player list of udphp but not more.
**      NOTE: This doesn't check if udphp is enabled!
**  
**  Usage:
**      <See above>
**
...
htme_debugger("htme_serverSyncPlayersUDPHP",htme_debug.DEBUG,"SERVER: Syncing player map and udphp player list");
...
for (var i=0;i<ds_list_size(self.udphp_playerlist);i++) {
    ...
    if (is_undefined(ds_map_find_value(self.playermap,player))) {
        htme_debugger("htme_serverSyncPlayersUDPHP",htme_debug.INFO,"SERVER: Registered a connection made by udphp.");
        //udphp and htme store the player information both as ip:port string, so we can use our functions to split it
        //register new player
...
for(var i=0; i<ds_map_size(self.playermap); i+=1) {
    ...
    if (ds_list_find_index(self.udphp_playerlist,key) == -1) {
        htme_debugger("htme_serverSyncPlayersUDPHP",htme_debug.INFO,"SERVER: Registered, that a client disconnected via udphp.");
